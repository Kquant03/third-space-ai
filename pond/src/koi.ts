...
export function toFrame(k: KoiState): KoiFrame {
  return {
    ...
    // Age in ticks since hatch. Surfaces to the diagnostic so the
    // operator can see real koi life-age (sim-days / sim-hours) and
    // distinguish that from snapshot freshness. The compute path
    // tickIntervalMs × ageTicks → sim-days happens on the client.
    at: k.ageTicks,
