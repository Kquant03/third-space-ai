...
// ───────────────────────────────────────────────────────────────────
//  Chat — visitor messages addressed to the pond
//
//  Visitor message lifecycle:
//    1. visitor sends text via chat_submit WS message
//    2. worker safety-filters via Gemma. On reject → ChatRejected back
//       to the submitting socket only
//    3. on accept: write to chatRing, broadcast via ChatMessageBroadcast
//       to every connected socket
//    4. periodically the pond itself observes (kind: "pond") when the
//       quiet/cooldown conditions described in pond-utterance.ts hold
//
//  The wire envelope mirrors the broadcast pattern used for ambient and
//  mechanism messages: a `t` discriminator on the outer object, fields
//  flattened alongside.
// ───────────────────────────────────────────────────────────────────

/** In-memory chat message kept in the DO's chatRing buffer. The wire
 *  envelope (ChatMessageBroadcast) adds the discriminator and rolling
 *  chatTotal counter. */
export interface ChatMessage {
  ...
  text: string;
  at: number;
  /** "visitor" for human-submitted messages; "pond" for self-observations
   *  generated by pond-utterance. */
  kind?: "visitor" | "pond";
}

/** Broadcast envelope sent to every connected socket when a visitor's
 *  message has passed safety filtering, or when the pond observes
 *  itself. */
export const ChatMessageBroadcastSchema = z.object({
  ...
  text: z.string(),
  at: z.number(),
  /** Monotonically-increasing total across the pond's lifetime —
   *  surfaces on the chat panel as a "this is message #N" affordance. */
  chatTotal: z.number().int().nonnegative(),
  kind: z.enum(["visitor", "pond"]),
});

export type ChatMessageBroadcast = z.infer<typeof ChatMessageBroadcastSchema>;

/** Sent back only to the submitting socket when a visitor's message
 *  fails safety filtering or other validation. The submitting client
 *  can use this to show "your message was not sent: <reason>" and
 *  optionally let the visitor try once more. */
...
export type ChatRejected = z.infer<typeof ChatRejectedSchema>;
