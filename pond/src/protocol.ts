...
// ───────────────────────────────────────────────────────────────────
//  Chat — the visitor-to-visitor surface (§ XIV)
//
//  Two strangers sit beside the pond at night and exchange a sentence.
//  The koi don't speak chat; the chat is *between visitors*, with the
//  pond as the place they happen to be co-present. Every chat message
//  passes through the same Gemma 4 E4B classifier used for pebble
//  inscriptions before it broadcasts — see safety.classifyVisitorContent.
//  Same family, same policy, different surface. (See § 7 of the paper.)
//
//  ChatMessage is the persistent shape: stored in the DO's in-memory
//  ring buffer (50 messages), echoed in the snapshot on connect,
//  broadcast to all sessions on accept. Server assigns id, handle, at.
// ───────────────────────────────────────────────────────────────────

/** A chat message as stored in the ring buffer and broadcast. */
export const ChatMessageSchema = z.object({
  id: z.string().min(1).max(64),
  /** Server-assigned visitor handle, sticky per session.
   *  Auto-generated in the koi-naming style: "Reed-Watcher",
   *  "Slow-Surface", etc. Set once at session open. */
  handle: z.string().min(1).max(40),
  text: z.string().min(1).max(200),
  at: z.number(),                  // unix ms
  /** "visitor" for messages submitted through the chat surface;
   *  "pond" for observations generated by the pond's own voice (see
   *  pond-utterance.ts). Optional for backward compat — absent means
   *  visitor. Client renders pond messages with distinct styling. */
  kind: z.enum(["visitor", "pond"]).optional(),
});

export type ChatMessage = z.infer<typeof ChatMessageSchema>;

/** Broadcast to every connected session when a chat message passes
 *  moderation, OR when the pond itself drops an observation. The id,
 *  handle, and at are server-assigned; for visitor messages only the
 *  `text` came from the visitor. */
export const ChatMessageBroadcastSchema = z.object({
   ...
   *  including this one. Monotonically increasing, persisted across
   *  DO restarts. Used by the collapsed-chat pill to show the
   *  all-time count rather than the ring-buffer count. */
  chatTotal: z.number().int().nonnegative().optional(),
  /** "visitor" or "pond" — see ChatMessageSchema.kind. */
  kind: z.enum(["visitor", "pond"]).optional(),
});

/** Sent back ONLY to the submitting socket when the classifier rejects.
 *  Other visitors never see the rejected text. */
export const ChatRejectedSchema = z.object({
  t: z.literal("chat_rejected"),
  /** Echo back what the visitor submitted, so their client can show
   *  "this was not sent: [text]" and offer a rewrite. */
  text: z.string().max(500),
  /** Human-readable reason from safety.classifyVisitorContent. */
...
export const SnapshotMessageSchema = z.object({
  ...
  /** Recent chat buffer (up to 50 messages). Lets a newly-connecting
   *  visitor see the conversation already in progress. */
  chat: z.array(ChatMessageSchema).optional(),
  /** Your auto-generated visitor handle for this session. The client
   *  shows this in the chat input as "speaking as <handle>". */
  yourHandle: z.string().min(1).max(40).optional(),
  /** Cumulative all-time count of chat messages this pond has ever
   *  accepted. Snapshot establishes baseline; subsequent chat_message
   *  broadcasts also carry this value for continuous sync. */
  chatTotal: z.number().int().nonnegative().optional(),
});
...
/** Visitor types a chat message to other visitors at the pond.
 *  Server-side: rate-limit check → moderation classifier (Gemma 4 E4B
 *  via safety.classifyVisitorContent) → on accept, attach server-assigned
 *  id/handle/at and broadcast; on reject, send chat_rejected back to
 *  this socket only. (§ XIV) */
